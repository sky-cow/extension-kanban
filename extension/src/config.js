...
export const CONFIG = {
  ...
  // Google OAuth client id (for extension oauth2 entry in manifest)
  GOOGLE_CLIENT_ID:
...
function validateConfig() {
  ...
  const missing = required.filter(
    (key) =>
      ...
      CONFIG[key].includes("XXXXXXXXX"),
  );

  if (missing.length > 0) {
    console.warn(
      "⚠️ Configuration incomplete. Please update the following in config.js:",
      missing,
    );
  }
}
